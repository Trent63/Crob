...
# Оставьте пустым, чтобы не спрашивать. Задайте порт, если ставите старую версию.
...
choose_version() {
    ...
    if ! tags_raw=$(fetch_tags); then
        ...
        read -rp "Введите версию вручную [по умолчанию: $DEFAULT_TAG]: " manual
    ...
    while true; do
        ...
        choice="${choice:-1}"
        if [[ "$choice" == "0" ]]; then
            read -rp "Введите версию: " REMNANODE_TAG
...
choose_port() {
    ...
    while true; do
        read -rp "$prompt [по умолчанию: $default]: " input
        input="${input:-$default}"
...
TPL
    local input
    while true; do
        read -rp "Выберите шаблон [по умолчанию: $DEFAULT_TEMPLATE]: " input
...
input_secret_key() {
    ...
    while true; do
        read -rp "Вставьте SECRET_KEY: " RAW_KEY
...
# --- Домен ---
read -rp "Введите домен (например nl115.croboxvps.com): " CERT_DOMAIN
...
# XTLS_PORT — только если явно включено (для нод <= 2.7.x)
XTLS_PORT=""
...
# --- Открытие портов (443 для ACME + нода + selfsteal, + XTLS если задан) ---
